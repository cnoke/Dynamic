...
public class RulerView extends View {
    ...
    private int shownub = 9;
    private OnEndResultListener listener;
    ...
    public void setOnEndResultListener(OnEndResultListener listener){
        this.listener = listener;
    }

    private void setAttr(AttributeSet attrs, int defStyleAttr) {

        TypedArray a = getContext().getTheme().obtainStyledAttributes(attrs, R.styleable.common_RulerView, defStyleAttr, 0);

        scaleLimit =  a.getInt(R.styleable.common_RulerView_common_scaleLimit, scaleLimit);

        rulerHeight = a.getDimensionPixelSize(R.styleable.common_RulerView_common_rulerHeight, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, rulerHeight, getResources().getDisplayMetrics()));

        rulerToResultgap = a.getDimensionPixelSize(R.styleable.common_RulerView_common_rulerToResultgap, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, rulerToResultgap, getResources().getDisplayMetrics()));

        scaleCount = a.getInt(R.styleable.common_RulerView_common_scaleCount, scaleCount);

        scaleGap = a.getDimensionPixelSize(R.styleable.common_RulerView_common_scaleGap, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, scaleGap, getResources().getDisplayMetrics()));

        minScale = a.getInt(R.styleable.common_RulerView_common_minScale, minScale)/scaleLimit;

        firstScale = a.getFloat(R.styleable.common_RulerView_common_firstScale, firstScale)/scaleLimit;

        maxScale = a.getInt(R.styleable.common_RulerView_common_maxScale, maxScale)/scaleLimit;

        bgColor = a.getColor(R.styleable.common_RulerView_common_bgColor, bgColor);

        smallScaleColor = a.getColor(R.styleable.common_RulerView_common_smallScaleColor, smallScaleColor);

        midScaleColor = a.getColor(R.styleable.common_RulerView_common_midScaleColor, midScaleColor);

        largeScaleColor = a.getColor(R.styleable.common_RulerView_common_largeScaleColor, largeScaleColor);

        scaleNumColor = a.getColor(R.styleable.common_RulerView_common_scaleNumColor, scaleNumColor);

        resultNumColor = a.getColor(R.styleable.common_RulerView_common_resultNumColor, resultNumColor);

        unitColor = a.getColor(R.styleable.common_RulerView_common_ruler_unitColor, unitColor);
        shownub = a.getInt(R.styleable.common_RulerView_common_shownub, shownub);

        String tempUnit = unit;
        unit = a.getString(R.styleable.common_RulerView_common_ruler_unit);
        if (TextUtils.isEmpty(unit)) {
            unit = tempUnit;
        }

        smallScaleStroke = a.getDimensionPixelSize(R.styleable.common_RulerView_common_smallScaleStroke, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, smallScaleStroke, getResources().getDisplayMetrics()));

        midScaleStroke = a.getDimensionPixelSize(R.styleable.common_RulerView_common_midScaleStroke, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, midScaleStroke, getResources().getDisplayMetrics()));
        largeScaleStroke = a.getDimensionPixelSize(R.styleable.common_RulerView_common_largeScaleStroke, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, largeScaleStroke, getResources().getDisplayMetrics()));
        resultNumTextSize = a.getDimensionPixelSize(R.styleable.common_RulerView_common_resultNumTextSize, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, resultNumTextSize, getResources().getDisplayMetrics()));

        scaleNumTextSize = a.getDimensionPixelSize(R.styleable.common_RulerView_common_scaleNumTextSize, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, scaleNumTextSize, getResources().getDisplayMetrics()));

        unitTextSize = a.getDimensionPixelSize(R.styleable.common_RulerView_common_unitTextSize, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, unitTextSize, getResources().getDisplayMetrics()));


        showScaleResult = a.getBoolean(R.styleable.common_RulerView_common_showScaleResult, showScaleResult);
        isBgRoundRect = a.getBoolean(R.styleable.common_RulerView_common_isBgRoundRect, isBgRoundRect);
    ...
    private void drawScaleAndNum(Canvas canvas) {
            ...
            if (valueAnimator != null && !valueAnimator.isRunning()) {      //手指抬起，并且当前没有惯性滑动在进行，创建一个惯性滑动
                ...
                valueAnimator.addListener(new AnimatorListenerAdapter() {       //增加一个监听，用来返回给使用者滑动结束后的最终结果刻度值
                    ...
                    public void onAnimationEnd(Animator animation) {
                        ...
                        if(listener != null){
                            listener.onEndResult(resultText);
                        }
                    }
                });
        ...
        canvas.translate(num2, 0);    //加该偏移的话，滑动时刻度不会落在0~1之间只会落在整数上面
        ...
        resultText = nf.format(currentScale);
        ...
        while (rulerRight < width) {
            if (num1 % scaleCount == 0) {    //绘制整点刻度以及文字
                if ((moveX >= 0 && rulerRight < moveX) || width / 2 - rulerRight <= getWhichScalMovex(maxScale + 1) - moveX) {
                    //当滑动出范围的话，不绘制，去除左右边界
                } else {
                    //绘制刻度，绘制刻度数字
//                    canvas.drawLine(0, 0, 0, midScaleHeight, midScalePaint);
                    float nowNub = (num1 / scaleCount + minScale)*scaleLimit;
                    float poor = 2 * (currentScale - nowNub) / (float)(shownub  + 1);
                    if(shownub % 2 == 0){
                        poor = 2 * nowNub / (float)shownub;
                    }
                    ...
                    float radius = 6;
                    if(nowNub == currentScale){
                        radius = 8;
                    }

                    midScalePaint.setAlpha((int)(poor * 255));

                    if(poor == 1){
                        scaleNumPaint.setTextSize(scaleNumTextSize + 2);
                    }else{
                        scaleNumPaint.setTextSize(scaleNumTextSize);
                    }
                    scaleNumPaint.setAlpha((int)(poor * 255));


                    canvas.drawCircle(0, rulerHeight / 8, radius, midScalePaint);
                    scaleNumPaint.getTextBounds(num1 / scaleGap + minScale + "", 0, (num1 / scaleGap + minScale + "").length(), scaleNumRect);
                    canvas.drawText((int)nowNub + "", -scaleNumRect.width() / 2, lagScaleHeight +
