...
public class RouteProcessor extends BaseProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        if (CollectionUtils.isNotEmpty(annotations)) {
            ...
            try {
                ...
                paresARoute(roundEnv.getElementsAnnotatedWith(Route.class));
    ...
    private void paresARoute(Set<? extends Element> routeElements) {
        ClassName aClass = ClassName.get("com.knd.common.manager", "RouteManager");
        ClassName build = ClassName.get("com.knd.common.manager", "RouteManager.Build");
        ClassName aSubClass = ClassName.get("com.knd.common.route", "RouteDynamic");
        ClassName param = ClassName.get("com.knd.common.route", "Param");

        //TypeSpec 生成类，接口，或者枚举   生成BindMainActivity类
        TypeSpec.Builder typeSpec = null;

        MethodSpec.Builder nameMethod = MethodSpec.methodBuilder("getName")
                .addModifiers(Modifier.PUBLIC)
                .addAnnotation(Override.class)
                .returns(String.class);//返回值  void

        MethodSpec.Builder routeMethod = MethodSpec.methodBuilder("route")
                .addModifiers(Modifier.PUBLIC)
                .addAnnotation(Override.class)
        ...
        CodeBlock.Builder routeIoc = CodeBlock.builder();//创建一段代码块
        routeIoc.addStatement("RouteManager.Build build = routeBuild(key,param)");
        routeIoc.beginControlFlow("if (build != null)");
        routeIoc.addStatement("build.navigation()");
        routeIoc.endControlFlow();
        routeMethod.addCode(routeIoc.build());

        MethodSpec.Builder routeBuildMethod = MethodSpec.methodBuilder("routeBuild")
                .addModifiers(Modifier.PUBLIC)
                .addAnnotation(Override.class)
                .addParameter(String.class,"key")
                .addParameter(param,"param")
                .returns(build);//返回值  void


        CodeBlock.Builder routeBuildIoc = CodeBlock.builder();//创建一段代码块
        routeBuildIoc.addStatement("RouteManager.Build build = null");
        routeBuildIoc.beginControlFlow("switch (key)");

        int i = 0;
        int size = routeElements.size() - 1;
        for (Element page : routeElements) {

            Route route = page.getAnnotation(Route.class);
            String group = getGroup(route.path());
            if(typeSpec == null){
                if(!isEmpty(group)){
                    typeSpec = TypeSpec.classBuilder("ARouteManager$$"+group)
                            .addModifiers(Modifier.PUBLIC)
                            .superclass(aSubClass);
                    nameMethod.addStatement("return $S",group);
                }
            }
            String methodName = getMethod(route.path());
            ...
            MethodSpec.Builder routeSpecBuilder = MethodSpec.methodBuilder(methodName)
                    .addModifiers(Modifier.PRIVATE)//public static
                    .addParameter(String.class,"key")
                    .addParameter(param,"param")
                    .returns(build);//返回值

            String routeKey = "return $T.getInstance().routerBuild(key)";
            boolean first = true;
            for(Element element : page.getEnclosedElements()){
                ...
                if(autowired.required()){
                    routeSpecBuilder.addStatement("if(param == null || $L == null) return null" , "param."+autowired.name());
                }
                if(first){
                    routeKey += ".addParams(";
                    first = false;
                ...
                routeKey += "\""+autowired.name()+"\", param." + autowired.name();

            }

            if(routeKey.endsWith(",")){
                routeKey.substring(0,routeKey.length() -1);
            }
            if(!first){
                routeKey += ")";
            }
            routeSpecBuilder.addStatement(routeKey,aClass);
            typeSpec.addMethod(routeSpecBuilder.build());


            routeBuildIoc.add("case $S: \n", route.path());
            routeBuildIoc.addStatement("build = $L(key,param)",methodName);
            routeBuildIoc.addStatement("break");

            if(i == size){

                routeBuildIoc.addStatement("default: \n build = defaultRoute(key,param)",aClass);
                routeBuildIoc.addStatement("break");
            }
            i ++;
        }
        ...
        routeBuildIoc.endControlFlow();
        routeBuildIoc.addStatement("return build");
        routeBuildMethod.addCode(routeBuildIoc.build());

        if(typeSpec == null){
            return;
        }
        typeSpec.addMethod(routeMethod.build());
        typeSpec.addMethod(routeBuildMethod.build());
        typeSpec.addMethod(nameMethod.build());
        //用于构造输出包含一个顶级类的Java文件
        JavaFile file = JavaFile.builder("com.knd.common.route.manager",typeSpec.build()).build();
        try {
            file.writeTo(mFiler);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public boolean isEmpty(CharSequence var0) {
